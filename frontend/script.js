(() => {
  ...
  function getStickyHeaderOffset() {
    const header = document.querySelector(".ta-nav") || document.querySelector("header");
    return (header ? header.offsetHeight : 0) + 20;
  }

  function scrollToLandingSection(sectionId, { updateHash = true } = {}) {
    ...
    document.querySelectorAll(".ta-nav-section-link").forEach((link) => {
  ...
  function setupLandingSectionNav() {
    ...
    if (initialHash && document.getElementById(initialHash)) {
      window.setTimeout(() => scrollToLandingSection(initialHash, { updateHash: false }), 120);
    }
  }
  ...
  function setupScroll3DEffects() {
    ...
    const scroll3dTargets = document.querySelectorAll(
      ".ta-hero, .ta-feature-card, .ta-how, .ta-benefit-card, .ta-cta, .ta-step, .ta-step-detail"
    );
    const blobs = document.querySelectorAll(".ta-bg-blobs .blob");
