...
public class AuthService {
    ...
    private static final Set<String> ALLOWED_GOOGLE_AUTH_MODES = Set.of("LOGIN", "SIGNUP");
    ...
    public LoginResponse googleAuth(GoogleAuthRequest request) {
        ...
        String normalizedEmail = email.trim().toLowerCase(Locale.ROOT);
        String authMode = normalizeGoogleAuthMode(request == null ? null : request.authMode);
        String normalizedRole = normalizeRequestedRole(request == null ? null : request.role);
        User byProviderId = userRepository.findByProviderId(providerId).orElse(null);
        User byEmail = userRepository.findByEmail(normalizedEmail).orElse(null);
        User user;

        if ("LOGIN".equals(authMode)) {
            user = byProviderId != null ? byProviderId : byEmail;
            if (user == null) {
                throw new BadRequestException("No account found for this Google email. Please register first.");
            }
        } else {
            if (byProviderId != null || byEmail != null) {
                throw new BadRequestException("Account already exists. Please login instead.");
            }
            user = createGoogleUser(normalizedEmail, fullName, providerId, normalizedRole);
        }
        ...
        if (user.getProviderId() == null || user.getProviderId().isBlank()) {
            ...
            throw new BadRequestException("This Google account is linked to a different user.");
        }
    ...
    private String normalizeGoogleAuthMode(String authMode) {
        String mode = authMode == null ? "LOGIN" : authMode.trim().toUpperCase(Locale.ROOT);
        if (!ALLOWED_GOOGLE_AUTH_MODES.contains(mode)) {
            throw new BadRequestException("Invalid authMode. Allowed values: LOGIN, SIGNUP");
        }
        return mode;
    }
