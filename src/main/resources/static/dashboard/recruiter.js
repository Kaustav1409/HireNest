...
/** Avoid mojibake and special dashes in UI copy (use plain ASCII). */
...
function flattenCandidatesByJob(byJob) {
  const rows = [];
...
function jobMapById(jobs) {
  return new Map((jobs || []).map((j) => [String(j.id), j]));
}

function missingSkillsForCandidate(c, job) {
  if (Array.isArray(c.missingSkills) && c.missingSkills.length) {
    return c.missingSkills.map((x) => String(x).trim()).filter(Boolean);
  }
  const required = parseSkillsCsv(job?.requiredSkills || "");
  const have = new Set(parseSkillsCsv(c.skills || ""));
  return required.filter((sk) => !have.has(sk));
}

/** Build insight cards from live dashboard data (ignore API text to avoid encoding issues). */
function buildRecruiterAiInsights(byJob = {}, applications = [], jobs = []) {
  const jobById = jobMapById(jobs);
  const pool = flattenCandidatesByJob(byJob);
  const insight = {
    topCandidateForJob: null,
    mostCommonMissingSkill: "",
    strongestSkillCluster: "",
    hiringBottleneck: ""
  };

  if (pool.length) {
    const top = pool.reduce(
      (best, c) => ((Number(c.matchScore) || 0) > (Number(best.matchScore) || 0) ? c : best),
      pool[0]
    );
    insight.topCandidateForJob = {
      candidateName: normalizeDisplayText(top.name),
      jobTitle: normalizeDisplayText(jobById.get(String(top.jobId))?.title || "Job"),
    ...
    pool.forEach((c) => {
      ...
      missingSkillsForCandidate(c, job).forEach((sk) => {
        const key = String(sk).trim().toLowerCase();
        if (!key) return;
    ...
    if (skillCounts.size) {
      ...
      insight.mostCommonMissingSkill = prettifySkillLabel(topSkill);
    }

    const overlapSkills = parseSkillsCsv(top.skills);
    if (overlapSkills.length) {
      insight.strongestSkillCluster = overlapSkills.slice(0, 3).map(prettifySkillLabel).join(", ");
    } else if (top.skillsOverlapCount != null && top.jobRequiredSkillCount != null) {
      insight.strongestSkillCluster = `${top.skillsOverlapCount} of ${top.jobRequiredSkillCount} required skills aligned on top match`;
    }
  }

  const apps = applications || [];
  if (!(jobs || []).length) {
    insight.hiringBottleneck = "No applications yet - post jobs to build your hiring pipeline.";
  } else if (!apps.length) {
    insight.hiringBottleneck = "No applications yet - post jobs to build your hiring pipeline.";
  } else if (!pool.length) {
    insight.hiringBottleneck = `${apps.length} application(s) on file. Ranked candidates will appear after matching runs.`;
  } else {
    const applied = apps.filter((a) => safeUpper(a.status) === "APPLIED").length;
    const shortlisted = apps.filter((a) => safeUpper(a.status) === "SHORTLISTED").length;
    const rejected = apps.filter((a) => safeUpper(a.status) === "REJECTED").length;
    if (applied >= 3 && shortlisted < Math.max(1, Math.floor(applied / 2))) {
      insight.hiringBottleneck =
        "Bottleneck: many candidates remain in Applied with few shortlists. Review first-screening SLAs.";
    } else if (rejected > shortlisted && rejected >= 2) {
      insight.hiringBottleneck = "High rejection rate. Tune job requirements or screening criteria.";
    } else if (shortlisted > 0) {
      insight.hiringBottleneck = `Pipeline active: ${shortlisted} shortlisted of ${apps.length} total applications.`;
    } else {
      insight.hiringBottleneck = `${apps.length} application(s) received. Move strong matches to Shortlisted.`;
    }
  }
...
function setRecruiterAiInsightsLoading() {
  const host = document.getElementById("recruiterAiInsights");
  if (!host) return;
  host.innerHTML = `<div class="card ai-insight-card"><p class="ai-insight-body muted">Loading insights...</p></div>`.repeat(4);
}
...
function renderRecruiterAiInsights(ai = {}) {
  ...
  const topLine =
    top.candidateName && top.jobTitle
      ? `${escapeHtml(normalizeDisplayText(top.candidateName))} · ${escapeHtml(normalizeDisplayText(top.jobTitle))} - ${top.matchScore != null ? top.matchScore + "%" : "N/A"} match`
      : "No ranked candidates yet - complete assessments and apply for jobs to appear here.";
  const common = ai.mostCommonMissingSkill
    ? escapeHtml(normalizeDisplayText(ai.mostCommonMissingSkill))
    : "No skill gap data available yet.";
  const cluster = ai.strongestSkillCluster
    ? escapeHtml(normalizeDisplayText(ai.strongestSkillCluster))
    : "Skill cluster insights will appear after candidate assessments.";
  const bottleneck = ai.hiringBottleneck
    ? escapeHtml(normalizeDisplayText(ai.hiringBottleneck))
    : "No applications yet - post jobs to build your hiring pipeline.";
  host.innerHTML = `
...
async function loadRecruiterData() {
  ...
  pipeEl.innerHTML = "";
  renderRecruiterAiInsights(buildRecruiterAiInsights(byJob, applications, jobs));
